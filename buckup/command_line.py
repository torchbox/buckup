...
class BuckupCommandLineInterface(CommandLineInterface):
    def __init__(self, boto3_profile=None, boto3_region=None):
        self.bucket_creator = BucketCreator(
            profile_name=boto3_profile, region_name=boto3_region
        )
        self.data = {}
    ...
    def print_account_information(self):
        try:
            ...
            print(
                "Credentials not set. Please make sure your AWS credentials "
                "are accessible by boto3."
            )
            print(
                'You can use "aws configure" command or set them using '
                'environment variables \n"AWS_ACCESS_KEY_ID" or '
                '"AWS_SECRET_ACCESS_KEY".'
            )
            print(
                "If you have already set AWS CLI credentials for another "
                'profile, you can use\n"--profile" flag to access them.'
            )
            print(
                "Read more: https://boto3.readthedocs.io/en/latest/guide/"
                "configuration.html"
            )
    ...
    def ask_summary(self):
        ...
        answer = self.ask_yes_no(
            "Do you want to create a bucket with the above details?"
        )
        if not answer:
            print("Cancelled.")
            sys.exit(130)
    ...
    def ask_public_get_object(self):
        ...
        while True:
            ...
            if "*" in paths and len(paths) > 1:
                print(
                    "If you specify a wildcard, you should not specify other paths.\n"
                )
                continue
            elif paths:
                break
...
def parse_args():
    ...
    parser.add_argument("--profile", type=str, help="AWS CLI profile you want to use")
    parser.add_argument(
        "--region",
        type=str,
...
def main():
    try:
        args = parse_args()
        cli = BuckupCommandLineInterface(
            boto3_profile=args.profile, boto3_region=args.region
        )
        cli.execute()
